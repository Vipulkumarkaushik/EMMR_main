...
def Creat_child1(moead):
    Dict_candidate_item_for_all = moead.candidate

    child = random.sample(Dict_candidate_item_for_all, 10)

    return child

def Creat_child2(x):
    user_index = _moead.user_index
    Dict_candidate_item_for_all = _moead.Dict_candidate_item_for_all
    child = []
    for a, i in enumerate(user_index):
        gene_i = []
        for k in range(len(_moead.label_dict)):
            candidate = list(set(Dict_candidate_item_for_all[i]) & set(_moead.label_dict[k+1]))
            pre_best = heapq.nlargest(int((len(_moead.label_dict[k+1]) * _moead.recommendation_len) ** 0.5), candidate, key=_moead.predict_rating[a].take)
            # candidate_dict = {}
            # for item in candidate:
            #     candidate_dict[item] = moead.predict_rating[a][item]
            # pre_best = heapq.nlargest(int((len(moead.label_dict[k+1]) * moead.recommendation_len) ** 0.5), candidate_dict, key=candidate_dict.get)
            if len(pre_best) > 0:
                gene_i.append(random.choice(pre_best))
            else:
                gene_i.append(random.choice(candidate))
        child += gene_i

    return child

# def Creat_child(moead):
#     user_index = moead.user_index
#     Dict_candidate_item_for_all = moead.Dict_candidate_item_for_all
#     child = []
#     for i in user_index:
#         gene_i = []
#         for k in range(len(moead.label_dict)):
#             candidate = list(set(Dict_candidate_item_for_all[i]) & set(moead.label_dict[k+1]))
#             if len(candidate) > 0:
#                 gene_i.append(random.choice(candidate))
#             else:
#                 gene_i.append(random.choice(moead.label_dict[k+1]))
#         child += gene_i
#
#     return child


def Create_Pop1(sizes, moead):
    Pop = []
    if sizes < 1:
        print('error in create_Pop')
        return -1
    # while len(Pop) != sizes:
    for _ in range(sizes):
        X = Creat_child1(moead)
        Pop.append(X)
    Pop = np.array(Pop)
    return Pop


def Create_Pop2(sizes, moead):
    if sizes < 1:
        print('error in create_Pop')
        return -1
    global _moead
    ...
    # pt = partial(Creat_child2, moead)
    Pop = p.map(Creat_child2, range(sizes))
    # Pop =list(tqdm(p.imap(pt, range(sizes)), total=sizes))
    p.close()
    p.join()
    Pop = np.array(Pop)
    return Pop
...
"""
    ...
    def initChrom(self, NIND=None, moea=None):
        ...
        self.Chrom = Create_Pop1(self.sizes, moea)
